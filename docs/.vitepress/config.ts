...
/** 侧边栏条目：与 add_audios.md 顶部的 Language Switch 一致，EN / 中文 分开展示 */
function subIcon(emoji: string, slug: string) {
    return `<span class="icon sub-icon">${emoji}</span>${slug}`
}

function docEn(slug: string, emoji: string) {
    return { text: subIcon(emoji, slug), link: `/docs/${slug}` }
}

function docZh(slug: string, emoji: string) {
    return { text: subIcon(emoji, slug), link: `/docs/${slug}.zh` }
}

function bilingualCategory(
    title: string,
    slugs: string[],
    emoji: string,
    options?: { zhExclude?: Set<string> }
) {
    const zhExclude = options?.zhExclude ?? new Set<string>()
    return {
        text: title,
        collapsed: false,
        items: [
            {
                text: 'English',
                collapsed: true,
                items: slugs.map((s) => docEn(s, emoji)),
            },
            {
                text: '简体中文',
                collapsed: true,
                items: slugs.filter((s) => !zhExclude.has(s)).map((s) => docZh(s, emoji)),
            },
        ],
    }
}
...
export default defineConfig({
    ...
    themeConfig: {
        ...
        // 侧边栏：按功能分类；每类下分 English / 简体中文（与各页 Language Switch 对应）
        sidebar: [
            {
                ...
                items: [
                    ...
                    bilingualCategory(
                        ...
                        '🔍',
                        { zhExclude: new Set(['get_text_effects']) }
                    ),
                    bilingualCategory(
