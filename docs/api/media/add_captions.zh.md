---
description: 向草稿批量添加字幕。captions 为 JSON 字符串，时间为微秒。
---
...
必须传入 create_draft 返回的 draft_url。captions 必须是 JSON 字符串（不是对象数组）。start/end 单位为微秒。
...
- [生成字幕信息](/docs/caption_infos)
- [获取文字动画](/docs/get_text_animations)
- [获取花字效果](/docs/get_text_effects)
