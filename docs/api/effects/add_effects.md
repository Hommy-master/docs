# ADD_EFFECTS API Documentation

## 🌐 Language Switch
[中文版](/docs/add_effects.zh) | [English](/docs/add_effects)

## Interface Information

```
POST /openapi/capcut-mate/v1/add_effects
```

## Function Description

Add video effects to existing drafts. This interface is used to add effect materials to Jianying drafts within specified time periods, supporting multiple effect types such as border effects, filter effects, and dynamic effects. Effects can be used to enhance the visual impact of videos.

## More Documentation

📖 For more detailed documentation and tutorials, please visit: [https://docs.jcaigc.cn](https://docs.jcaigc.cn)

## Request Parameters

```json
{
  "draft_url": "https://capcut-mate.jcaigc.cn/openapi/capcut-mate/v1/get_draft?draft_id=2025092811473036584258",
  "effect_infos": "[{\"effect_title\": \"录制边框 III\", \"start\": 0, \"end\": 5000000}, {\"effect_title\": \"简约边框\", \"start\": 2000000, \"end\": 7000000}]"
}
```

### Parameter Description

| Parameter | Type | Required | Default | Description |
|-----------|------|----------|---------|-------------|
| draft_url | string |✅ | - | Complete URL of the target draft |
| effect_infos | string |✅ | - | JSON string of effect information list |

### Parameter Details

#### effect_infos Array Structure

`effect_infos` is a JSON string containing an array of effect objects, each with the following fields:

```json
[
  {
    "effect_title": "录制边框 III",  // Effect name/title, required parameter
    "start": 0,                              // Effect start time (microseconds), required parameter
    "end": 5000000                           // Effect end time (microseconds), required parameter
  }
]
```

**Field Description**:
- `effect_title`: Effect name; must be one of the Supported Effect Names below
- `start`: Effect start time in microseconds
- `end`: Effect end time in microseconds

#### Time Parameters

- **start**: Start time of the effect on the timeline, unit microseconds (1 second = 1,000,000 microseconds)
- **end**: End time of the effect on the timeline, unit microseconds
- **Duration**: Effect duration = end - start

<!-- EFFECT_LIST_START -->
### Supported Effect Names (`effect_title` values)

Use the names below directly as `effect_title` (same as CapCut/Jianying display names; from scene effects and character effects). Unmatched names will fail.

Total: **1328** effects (1097 scene + 240 character, after dedupe):

```text
1998
2024边框
2025气球泡泡
360运镜
360运镜 II
3D兔兔
3D春联
3D环绕
3D环绕屏
70s
90s画质
betamax
Bling飘落
BOOM！
C300
CCD闪光
crash！
DV录制框
DV界面
emoji钻石
I Lose You
I Love You
Ins描边
ins界面
ins风放大镜
IXUS
JVC
kirakira
ktv灯光
ktv灯光 II
List边框
Love u
MV封面
New Year
PASSION
PS边框
RGB描边
S形运镜
VCR
W830
windows弹窗关闭
windows弹窗打开
X
X-Signal
X开幕
X瞬移
一刀两断
丁达尔光线
丁达尔旋焦
万向镜像
万圣emoji
万圣夜
三屏
三格漫画
三段式黑白
上上签
下雨
下雨涟漪
不对劲
不规则黑框
丝印涂鸦
丝滑运镜
两屏
两屏分割
中刀
中枪了
中轴旋转
主体冲破屏幕
乌鸦飞过
九尾狐
九屏
九屏跑马灯
二次元运镜
云朵绵绵
五彩纸屑
五彩缤纷
五星好评
交叉震闪
亮片
人影爆闪
人鱼滤镜
仙女变身
仙女变身 II
仙女棒
仙女棒II
仙尘闪闪
低保真
低像素
低像素 II
侧移模糊
信号干扰
倒带
倒计时
倒计时 II
倒计时 III
倒计时II
假日闪闪 Ⅱ
像素屏闪
像素扫描
像素拉伸 II
像素排序
像素故障
像素爱心
像素画
像素纹理
像素震闪
光斑虚化
光斑飘落
光晕
光晕 II
光环 I
光环 II
光线扫描
光线拖影
光谱扫描
兔个好彩头
兔兔碎闪
兔年红包
全剧终
全屏烟花
全屏祝福语
全息扫描
六屏
关月亮
冰冷实验室
冰霜
冰霜 II
冲击波
冲刺
冲刺 II
冲刺 III
冲屏闪粉
凄凉
几何图形
几何拖尾 I
几何拖尾 II
击中
刀光剑影
分区显色
分头行动
分屏开幕
分屏漏光
分身
分身 III
分身ll
初雪 I
剪纸窗花
剪纸边框
加载甜蜜
动态侦测
动态格
动感光束
动感变焦
动感扫光
动感推镜
动感模糊
动感爱心
动感竖线
动感色卡
动感荧光
动感蓝带
动感运镜
十字模糊
十字爆闪
单向移动
单彩渐变
单色填充
单色涂鸦
南瓜光斑
南瓜笑脸
卡机
卡通渲染
卡通脸
卷动
卷轴打开
厚卡纸
原相机
原相机拉近
原相机滤镜
原相机缩放
友友商店
双重辉光
反转片 I
发光
发光 II
发光HDR
发光分身
取景器
取景框
取景框 II
变形了
变彩色
变清晰
变清晰 II
变焦推镜
变焦烟花
变秋天
变老-美颜
变色狙击
变色闪光
变身
变速推镜
变速推镜II
变黑白
可爱圣诞
可爱女生
可爱涂鸦
可爱猪
可爱龙龙
吓到失魂
吸血鬼
吻痕坏笑
告白氛围
咔嚓
哈哈哈
哈哈弹幕
哈哈镜
哈苏胶片
哥特
唱片
唱片封面
啊啊啊啊
嘻哈眼镜
噪点
噪点屏闪
噪片映射
四屏
回弹摇摆
回忆文件夹
回忆胶片
图腾
圆形分屏
圆形烟花
圆形监控
圆形虚线放大镜
圣诞CD
圣诞Kira
圣诞光斑
圣诞小熊
圣诞帽
圣诞日记
圣诞明信片
圣诞星光
圣诞树
圣诞胡子
圣诞辣妹
圣诞边框
圣诞铃铛
圣诞限定
地狱使者
基础黑框
塑料封面
塑料封面 II
塑料封面III
声波
复古DV
复古DV II
复古DV III
复古DV IV
复古半调
复古发光
复古多格
复古弹窗 I
复古弹窗 II
复古彩虹
复古拼贴
复古撕纸
复古漫画
复古甜心
复古电视
复古电视机
复古碎钻
复古紫调
复古红调
复古蓝调
复古辉光
复古边框
复古连拍
复古闪闪
复古频闪
夏日冰块
夏日泡泡 I
夕阳
夕阳 II
夕阳 III
多屏圣诞树
夜蝶
夜视框
大头
大字祝福
大爆竹
大眼睛
大雪
大雪纷飞
天使
天使光
天使环
天使翅膀
天使降临
太阳神
失焦
失焦CCD
失焦光斑
夸夸弹幕
夸张信号
夺冠
奇行种
好吃
妖气
委屈丑丑脸
孔明灯
孔明灯 II
字幕投影
字幕投影 II
字幕投影 III
字幕投影 IV
定格祝福
定格闪烁
实况开幕
害羞
密码解锁
对焦DV
小剧场
小动物
小恶魔
小花花
小鹿角
少女心
少女心事
少女星闪
尴尬住了
局部扭曲
局部推镜
局部模糊
局部色彩
局部马赛克
居中闪切
屏幕律动
左右摇晃
巴哥犬
布拉格
帅气男生
幻动光斑
幻彩故障
幻彩文字
幻彩流光
幻影
幻影 I
幻影 II
幻影平移
幻术摇摆
幻觉
幻觉 II
幽灵
广告运镜
广角
庆祝彩带
开幕
开幕  II
弥散流光
弯曲故障
弹动摇镜
弹动旋入
弹性闪动
弹震运镜
强锐化
录像带
录像带 II
录像带 III
录制框
录制边框
录制边框 II
录制边框 III
录制边框 IIII
彩信
彩光幻影
彩光摇晃
彩光频闪
彩噪画质
彩带
彩带飘落
彩色像素
彩色描边
彩色流光 I
彩色流光 II
彩色流光 III
彩色漫画
彩色火焰
彩色珠滴
彩色电光
彩色碎彩
彩色碎片
彩色碎片 II
彩色碎闪
彩色负片
彩色重影
彩色闪烁
彩色雪花
彩虹光
彩虹光 II
彩虹光影
彩虹光晕
彩虹射线
彩虹幻影
彩虹棱镜
彩虹气泡
彩虹泛光
彩虹流体
彩虹爱心
彩虹边缘
彩虹闪屏
彩边频闪
彩钻
影分身
微信祝福
微笑摇摆头
微震闪黑
心动
心动信号
心心眼
心河
心花怒放
心花怒放III
心跳
心跳 II
心跳黑框
必杀技
必杀技 II
快速变焦
快闪运镜
怀旧画质
怀旧边框
怀旧边框 II
怦然心动
恐怖故事
恐怖故事 II
恐怖故事 III
恐怖涂鸦
恐怖综艺
恭喜发财
恭喜发财 II
恶灵冲屏
恶灵骑士
恶魔之翼
恶魔印记
恶魔尾巴
恶魔角
情绪定格
惨
意识流
愛
慢门拖影
憔悴
懵
我不听
我太可爱了
我服了
我爱了
我酸了
我麻了
手写描边
手写边框
手印涂抹
手帐边框
手持运镜
手电筒
手绘拍摄器
手绘描边
手绘边框 II
打击
打脸
扫描 II
扫描光条
扭动变焦
扭曲变焦
扭曲模糊
抖动
抖动模糊
折叠拜年
折痕
折痕 II
折痕 III
折痕 IV
折痕 V
报纸:今日热门
抽帧拖影
拉伸旋镜
拉扯震动
拉镜开幕
拍照定格
拍立得成像
拖影灯光
拟截图放大镜
拼贴抽帧
拼贴风暴
拽酷红眼
捕梦
掉小珍珠啦
推拉跟随
推拉运镜
推近推远
摇摆
摇摆 II
摇摇乐
摇晃叠影
摇晃推镜
摇晃运镜
撒星星
撒星星 II
撕纸定格
撕纸拼贴
撕纸涂鸦边框
撕纸特写
播放器
播放器 II
播放界面
擦拭开幕
放大镜
放大镜视角
放映机
放映机卡顿
放映机抖动
放映滚动
故障
故障 II
故障定格
故障开幕
故障描边 I
故障读条
故障震闪
散光弹动
散光闪烁
数字矩阵
数学公式
敲打
文字闪动
斑斓
斜向模糊
斜线震动
新年仙女棒
新年倒数
新年兔气球
新年开幕
新年快乐
新年投影
新年星黛露
新年烟花 II
新年的一封信
新年礼花
新年边框
新春唱片
新春杂志
新春海报
新春祝福
新春边框
方形取景器
方形开幕
方形模糊
旋焦
旋焦推镜
旋转分身
旋转变焦
旋转回弹
旋转圆球
旋转抖动
旋转抖动 II
旋转方块
无信号
无限穿越
日式DV
日文字幕
日系大头贴
日落灯
日进斗金
时光碎片
时尚涂鸦
时间停止
星光
星光 II
星光变焦
星光放射
星光绽放
星光闪耀
星光闪闪
星夜
星星冲屏
星星变焦
星星坠落
星星彩绘
星星投影
星星拖尾
星星灯
星星闪烁
星星闪烁 II
星星闪烁 III
星月童话
星河
星河 II
星火
星火 II
星火炸开
星移
星空
星辰
星辰 I
星辰 II
星辰 III
星雨
春日樱花
春日边框
晴天光线
暗夜
暗夜归来
暗夜彩虹
暗夜彩虹 II
暗夜彩虹III
暗夜精灵
暗夜蝙蝠
暗角
暗黑剪影
暗黑噪点
暗黑蝙蝠
曝光
曝光变焦
曝光扩散
曝光降低
曲线模糊
月亮投影
月亮闪闪
月光闪闪
有事吗？
望远镜
未来主义
未来眼镜
机械几何
机械姬 I
机械姬 II
机械环绕 I
机械环绕 II
机灵怪
杂志
极速旋转
柔和辉光
树影
树影 II
格纹纸质
格纹纸质 II
梦境
梦境 II
梦境 III
梦境 IV
梦幻辉光
梦幻雪花
梦蝶
梦魇
梵高背景
模拟拍照
模糊
模糊开幕
模糊星光
模糊星光 II
模糊闭幕
横向闪光
横向闭幕
横条开幕
横纹故障
横纹故障 II
樱花朵朵
樱花飘落
橘色负片
欧根纱
欧根纱II
欧美女性
欧美男性
毛刺
毛玻璃
气泡 I
气泡 II
气波
气炸了
气球满屏
气球花花
氛围大头贴
氛围边框
水光影
水墨晕染
水彩晕染
水波倒影
水波模糊
水波泛起
水波流动
水波纹
水波纹投影
水波负片
水滴扩散
水滴模糊
水滴滚动
水雾消散
沉沦
油画模糊
油画泼墨
油画纹理
油纸伞
法式暖调
法式涂鸦
泛光扫描
泛光爆闪
泛光闪动
泡泡
泡泡光斑
泡泡冲屏
泡泡变焦
波动清晰
波浪
波浪丝印
波点分身
波点彩绘
波纹扭曲
波纹色差
波纹闪动
流体冲屏
流体故障
流体荡开
流光描边
流光碎闪
流动烟雾
流口水
流星雨
浓雾
浪漫氛围
浪漫氛围 II
浪漫烟火
海报描边
海鸥DC
涂鸦vlog
涂鸦切割边框
涂鸦切开
涂鸦抽帧
涂鸦拍立得
涂鸦鬼影
液态分离
淡彩边框
清新绿格子
渐显开幕
渐渐放大
渐隐闭幕
温柔细闪
游戏界面
滑动打开
滑动载入
满屏问号
漂浮爱心
漏光噪点
漏光闪烁
漩涡
漩涡溶解
漫画负片
潮流入侵
潮流涂鸦
潮酷女孩
潮酷男孩
激光几何
瀑布开幕
瀑布烟花
火光
火光刷过
火光包围
火光翻滚
火光蔓延
火焰图腾
火焰拖尾
火焰环绕
火焰翅膀 I
火焰翅膀 II
灵异幻觉
灵机一动
灵魂出窍
灵魂出窍 II
灵魂出走
灿灿金币
灿金彩带
炫光变焦
炫光扫描
炫彩
炫彩 II
炸蛇烟花
点赞
烟火星河
烟花
烟花 II
烟花 III
烟花2024
烟花2025
烟花光晕
烟花孔明灯
烟花字
烟雾
烟雾炸开
热力光谱 I
热力光谱 II
热恋
焰火
熬夜冠军
爆元宝
爆炸
爆闪锐化
爱心
爱心bling
爱心Kira
爱心下雪
爱心光斑
爱心光斑 II
爱心光波
爱心发射
爱心啵啵
爱心射线
爱心扫光
爱心投影
爱心方块
爱心暗角
爱心气泡
爱心气球
爱心泡泡
爱心烟花Ⅱ
爱心焰火
爱心爆炸
爱心眼
爱心缤纷
爱心缤纷 II
爱心美瞳
爱心跳动
爱心跳动 II
爱心软糖
爱心边框
爱心闪烁
牛皮纸关闭
牛皮纸打开
牛皮纸边框 I
牛皮纸边框 II
狱火
狼人
猩猩脸
猫眼迎财
猫耳女孩
玫瑰冲屏
玫瑰花瓣
玻璃破碎
珠光Kira
珠光碎闪
琉璃拖影
甜心投影
生日快乐
生气
电光包围
电光描边
电光描边 II
电光放射
电光波动
电光漩涡
电光灼烧
电光爆闪
电光爆闪 II
电光爱心
电光眼
电光耳机
电击
电子复古粉
电子屏
电子屏故障
电影刮花
电影感
电影感画幅
电脑桌面
电视关机
电视开机
电视彩虹屏
电视纹理
电音故障
画展边框
画质清晰
白噪点边框
白胶边框
白色描边
白色渐显
白色爱心
白色线框
白色边框
白鸽
百叶窗
百叶窗 II
监控
盗梦空间
盛世美颜
相机抓拍
相机框抖动
相机网格
相片定格
相纸
真爱至上
真的会谢
真香
眼神光
瞬移
瞬间模糊
矩形闪白
矩阵频闪
破冰
破碎的心
碎片分身
碎闪描边
碎闪边缘
磁带DV
磨砂水晶
磨砂纹理
祝福环绕
神明少女
神龙纳福
福气满满
秋日暖阳
秋日暖黄
科技氛围 I
科技氛围 II
科技氛围 III
秘密
移形回位
移形幻影 I
移形幻影 II
移轴模糊
空气流体
空灵
窗格
窗格光
竖向开幕
竖向闪光
竖线屏闪
竖闪模糊
童趣闪电
笑哭
简约边框
箭头放大镜
箭头环绕
粉红老电视
粉红芭比边框
粉色便便
粉色闪粉
粉雪
粉黄渐变
粒子弥散
粒子放射
精灵闪粉
精细锐化
精致辉光
糖果纸
紫光夜
紫色波纹
紫色负片
紫雾
繁星点点
繁花棱镜II
红包来了
红包转转
红晕
红蓝魔
红边模糊
纵向开幕
纵向模糊
纵向跳动
纸膜边框 I
纸膜边框 II
纸质抽帧
纸质撕边
纸质边框
纸质边框 II
纸醉金迷
线光变速
细闪
细闪 II
细闪 III
绚丽光斑
缤纷
缤纷彩带
缤纷烟花
缩放运镜
网点丝印
美味召唤
美式
美式 II
美式 III
美式 IV
美式 V
美漫
群蝶飞舞
羽毛
羽毛飘落
翻转变焦
翻转开幕
老式DV
老照片
老照片 II
老照片 III
老电影
老电影 II
老电视卡顿
聚光灯
聚光灯金粉
聚焦
聚焦色散
背景拖影
背景氛围II
胡言乱语
胶卷
胶片
胶片 II
胶片 III
胶片 IV
胶片V
胶片冷绿
胶片抖动
胶片显影
胶片暖棕
胶片框
胶片框 II
胶片框 III
胶片滚动
胶片漏光
胶片漏光 II
胶片漏光III
胶片翻转
胶片边框
胶片连拍
胶片闪切
脉搏跳动
脸红
脸绿了
脸部故障
腾云驾雾
自然
自然 II
自然 III
自然 IV
自然 V
舞者
舞者 II
色差
色差开幕
色差放大
色差放射
色差故障
色差故障 II
色差星闪
色差震闪
色差默片
色散冲击
色散故障
节奏热成像
节日彩带
花屏故障
花屏故障 II
花火
花火 II
花瓣环绕
花瓣飘落
花瓣飞扬
荡漾 II
荡秋千
荧光扫描
荧光星河
荧光爱心
荧光线描
荧光绿
荧光蝙蝠
荧幕噪点
荧幕噪点 II
菱形光斑
菱形变焦
萤光
萤光飞舞
萤火
落叶
落叶 II
落樱
蒸汽波
蒸汽波投影
蒸汽波路灯
蒸汽腾腾
蓝光扫描
蓝光爆闪
蓝线模糊
蓝色丝印
蓝色负片
蓝色闪电边框
蔡国强烟花
虚化
虚拟人生 I
虚拟人生 II
虹光旋入
蛇形走位
蛇蛇涂鸦
蝙蝠Kira
蝴蝶
蝴蝶 II
蝴蝶光斑
蝴蝶翅膀
蝶舞
表面模糊
衰
裂开了
西海岸涂鸦
视线遮挡
视频分割
视频播放
视频界面
记忆存档
诡异分割
诡异故障
负片分屏
负片描边
负片涂鸦
负片涂鸦 II
负片涂鸦 III
负片游移
负片闪烁
负片频闪
负片频闪 II
财富满满
财富满满 II
贴身粒子
赛博朋克 I
赛博朋克 II
赛博眼镜
赞赞赞
超大光斑
超强锐化
跟随运镜
跟随运镜 II
跳色四宫格
蹦迪光
蹦迪彩光
车窗
车窗 II
车窗影
转运祝福
轮廓扫描
轻微抖动
轻微放大
轻微故障
轻金属
辉光开幕
边缘glitch
边缘加色
边缘加色 II
边缘加色 III
边缘发光
边缘扫光
边缘荧光
迎接2025
运动一夏
运动轨迹
迪斯科
迷幻分身
迷幻故障
迷幻烟雾
迷幻荡漾
迷幻震动
迷离
迷茫
迷雾
逆光对焦
选中框
邮票边框
重复变焦
重复震闪
金元宝
金属背景
金币掉落
金片
金片 II
金片炸开
金粉
金粉 II
金粉 III
金粉撒落
金粉旋转
金粉聚拢
金粉闪闪
金粉飘落
金色碎片
金色辉光
金色雪花
金蛇贺岁
金边闪烁
金银珠宝
金鱼漂浮
钻光
钻石碎片
铅笔涂鸦
银杏飘落
银蛇缠绕
镜像
镜头变焦
镭射眼 I
镭射眼 II
镭射边框
长虹玻璃
闪亮登场
闪亮登场 II
闪光弹跳
闪光放大
闪光灯 II
闪光灯 Ⅰ
闪光灯IV
闪光震动
闪动
闪动光斑
闪屏
闪影
闪烁
闪电
闪电扭曲
闪电炸裂
闪电环绕
闪电眼
闪白
闪白 II
闪粉胶片
闪耀星光
闪闪
闪闪发光 II
闪黑
闪黑II
闭幕
闭幕 II
阳光
阴云密布
阴暗面
随机色块
随机色块 II
随机裁剪
随机闪切
随机马赛克
隐形人
隔行DV
隔行扫描
难吃
难过
雨季 I
雨滴
雨滴2
雨滴晕开
雪窗
雪花
雪花光斑
雪花冲屏
雪花变色
雪花开幕
雪花故障
雪花散落
雪花眼泪
雪花细闪
雪雾
零帧起手
零点解锁
雾气
雾气 II
雾气光线
雾镜 II
震动
震动光束
震动发光
震动屏闪
震动扫光
震动推镜
震闪渐黑
霓虹光线
霓虹投影
霓虹摇摆
霓虹灯
霓虹爱心
霓虹特技
霓虹闪切
音符拖尾
音符拖尾 II
预警
颤抖
飓风
飘落花瓣
飘落闪粉
飘落闪粉 II
飘雪
飘雪 II
飞机窗
飞翔的帽子
飞速计算
马赛克
马赛克闪切
高光人物
高光瞬间
高速彩光
鬼火
魅力光束
魔法
魔法变身
魔法边框
魔法边框 II
鱼眼
鱼眼 II
鱼眼 III
鱼眼 IV
鱼眼星星
鱼眼涂鸦
鱼眼闪电
黄蓝星芒
黑人女孩
黑人男生
黑白VHS
黑白三格
黑白漫画
黑白漫画 II
黑白线描
黑白胶片
黑线故障
黑羽毛
黑羽毛 II
黑胶边框
黑色噪点
黑色眼泪
黑色老电视
鼻血
```
<!-- EFFECT_LIST_END -->

## Response Format

### Success Response (200)

```json
{
  "draft_url": "https://capcut-mate.jcaigc.cn/openapi/capcut-mate/v1/get_draft?draft_id=2025092811473036584258",
  "track_id": "effect-track-uuid",
  "effect_ids": ["effect1-uuid", "effect2-uuid"],
  "segment_ids": ["segment1-uuid", "segment2-uuid"]
}
```

### Response Field Description

| Field | Type | Description |
|-------|------|-------------|
| draft_url | string | Updated draft URL |
| track_id | string | Effect track ID |
| effect_ids | array | List of added effect IDs |
| segment_ids | array | List of segment IDs |

### Error Response (4xx/5xx)

```json
{
  "detail": "Error message description"
}
```

## Usage Examples

### cURL Examples

#### 1. Basic Effect Addition

```bash
curl -X POST https://capcut-mate.jcaigc.cn/openapi/capcut-mate/v1/add_effects \
  -H "Content-Type: application/json" \
  -d '{
    "draft_url": "YOUR_DRAFT_URL",
    "effect_infos": "[{\"effect_title\":\"录制边框 III\",\"start\":0,\"end\":10000000}]"
  }'
```

#### 2. Multiple Effects

```bash
curl -X POST https://capcut-mate.jcaigc.cn/openapi/capcut-mate/v1/add_effects \
  -H "Content-Type: application/json" \
  -d '{
    "draft_url": "YOUR_DRAFT_URL",
    "effect_infos": "[{\"effect_title\":\"录制边框 III\",\"start\":0,\"end\":5000000},{\"effect_title\":\"简约边框\",\"start\":3000000,\"end\":8000000}]"
  }'
```

#### 3. Sequential Effects

```bash
curl -X POST https://capcut-mate.jcaigc.cn/openapi/capcut-mate/v1/add_effects \
  -H "Content-Type: application/json" \
  -d '{
    "draft_url": "YOUR_DRAFT_URL",
    "effect_infos": "[{\"effect_title\":\"录制边框 III\",\"start\":0,\"end\":1000000},{\"effect_title\":\"简约边框\",\"start\":1000000,\"end\":9000000},{\"effect_title\":\"复古边框\",\"start\":9000000,\"end\":10000000}]"
  }'
```

## Error Code Description

| Error Code | Error Message | Description | Solution |
|------------|---------------|-------------|----------|
| 400 | draft_url is required | Missing draft URL parameter | Provide a valid draft URL |
| 400 | effect_infos is required | Missing effect information parameter | Provide valid effect information JSON |
| 400 | effect_infos format error | JSON format is incorrect | Check JSON string format |
| 400 | Effect configuration validation failed | Effect parameters do not meet requirements | Check parameters for each effect |
| 400 | effect_title is required | Effect title missing | Provide title for each effect |
| 400 | start is required | Effect start time missing | Provide start time for each effect |
| 400 | end is required | Effect end time missing | Provide end time for each effect |
| 400 | Time range invalid | end must be greater than start | Ensure end time is greater than start time |
| 400 | Effect title not found | Specified effect does not exist | Check if effect title is valid |
| 404 | Draft does not exist | Specified draft URL invalid | Check if draft URL is correct |
| 500 | Effect processing failed | Internal processing error | Contact technical support |

## Notes

1. **JSON Format**: effect_infos must be a valid JSON string
2. **Time Unit**: All time parameters use microseconds (1 second = 1,000,000 microseconds)
3. **Effect Names**: Effect titles must match exactly with system effect names
4. **Time Overlap**: Multiple effects can be applied to the same time period
5. **Effect Priority**: Effects are applied in the order they appear in the array
6. **Performance**: Complex effects may affect video processing performance
7. **Supported Effects**: System currently supports transition effects like "Fade In", "Fade Out", color filters, etc.
8. **Preview Limitation**: Effects may not be visible in preview but will be applied during final video generation

## Workflow

1. Validate required parameters (draft_url, effect_infos)
2. Parse effect_infos JSON string
3. Validate parameter configuration for each effect
4. Obtain and decrypt draft content
5. Create effect track
6. Add effect segments to track
7. Apply effects to video segments
8. Save and encrypt draft
9. Return processing result

## Related Interfaces

- [Create Draft](/docs/create_draft)
- [Add Videos](/docs/add_videos)
- [Add Images](/docs/add_images)
- [Save Draft](/docs/save_draft)
- [Generate Video](/docs/gen_video)

---

<div align="right">

📚 **Project Resources**  
**GitHub**: [https://github.com/Hommy-master/capcut-mate](https://github.com/Hommy-master/capcut-mate)  
**Gitee**: [https://gitee.com/taohongmin-gitee/capcut-mate](https://gitee.com/taohongmin-gitee/capcut-mate)

</div>