...
| transition | string |❌ | None |转场名称，可用值见 [add_videos](/docs/add_videos.zh) |
| transition_duration | number |❌ | 300000 |转场时长(微秒) |
| volume | number |❌ | 1.0 |音量大小(0.0-2.0) |

### 可用转场名称

转场完整可用值清单，请参见 [添加视频（add_videos）](/docs/add_videos.zh) 文档中的「支持的转场名称」。画面入场/出场/循环动画请参见 [添加图片（add_images）](/docs/add_images.zh)。
