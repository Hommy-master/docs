---
description: "Poll video export status: pending, processing, completed, or failed."
---

# GEN_VIDEO_STATUS API Documentation

## 🌐 Language Switch
[中文版](/docs/gen_video_status.zh) | [English](/docs/gen_video_status)
...
Query the status and progress of video generation tasks. Used together with the [gen_video](/docs/gen_video) interface to track the execution of video generation tasks in real-time, including task status, progress percentage, completion results, and other information.

<llm-only>

Poll every 3-5 seconds. Read video_url only when completed; read error_message when failed. Do not stop while pending or processing.
...
- **draft_url**: Complete URL of the draft, used to identify the video generation task to query status for
  ...
  - Acquisition Method: draft_url returned after submitting task via [gen_video](/docs/gen_video) interface
...
- [gen_video](/docs/gen_video) - Submit video generation task
- [create_draft](/docs/create_draft) - Create new draft file
- [save_draft](/docs/save_draft) - Save draft file
