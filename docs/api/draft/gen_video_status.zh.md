---
description: "查询视频导出进度。status 为 pending/processing/completed/failed。"
---
...
查询视频生成任务的状态和进度。配合 [gen_video](/docs/gen_video) 接口使用，用于实时跟踪视频生成任务的执行情况，包括任务状态、进度百分比、完成结果等信息。

<llm-only>

每 3-5 秒轮询一次。completed 时读取 video_url；failed 时读取 error_message。不要在 pending/processing 时停止。
...
- **draft_url**: 草稿的完整URL，用于标识要查询状态的视频生成任务
  ...
  - 获取方式：通过 [gen_video](/docs/gen_video) 接口提交任务后返回的draft_url
...
- [gen_video](/docs/gen_video) - 提交视频生成任务
- [create_draft](/docs/create_draft) - 创建新的草稿文件
- [save_draft](/docs/save_draft) - 保存草稿文件
