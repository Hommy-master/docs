---
description: 异步提交视频导出任务。仅此接口收费，线上调用需要 apiKey。
---
...
此接口只提交任务，不返回视频文件。接着轮询 gen_video_status，直到 status=completed 或 failed。线上导出请带 uuid 格式的 apiKey。
...
- **draft_url**: 草稿的完整URL地址
  ...
  - 获取方式：通过[创建草稿](/docs/create_draft)或[保存草稿](/docs/save_draft)接口获取
...
- [添加视频](/docs/add_videos)
- [添加音频](/docs/add_audios)
- [添加图片](/docs/add_images)
- [获取草稿](/docs/get_draft)
- [查询视频生成状态](/docs/gen_video_status)
