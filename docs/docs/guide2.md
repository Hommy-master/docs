...
在关键词混淆基础上，进一步对HTML文件中的句子和段落进行深度混淆：

#### 句子混淆方式
- **字符分割**: 将句子中的字符用span标签包装 `<span>文</span>字内容<span>分</span>割`
- **不可见字符注入**: 在句子中插入零宽字符 `文字&#8203;内容&#8205;分割`
- **混合字符混淆**: 组合span标签和不可见字符 `<span data-key="abc">文</span>字&#8203;内容`
- **保持语义**: 不改变句子内容和含义，只在视觉层面进行混淆

#### 智能句子处理
- **文本节点识别**: 精确识别HTML文档中的纯文本节点
- **智能跳过**: 自动检测并跳过已被关键词混淆的内容，避免重复处理
- **长度控制**: 智能处理10-200字符长度的句子片段
- **代码保护**: 自动跳过script、style、code等标签内容
- **随机混淆率**: 可配置句子混淆概率（默认60%）
- **层次处理**: 在关键词混淆之后执行，形成双重保护
...
# 高级混淆配置
node ./bin/obfuscate.js ./demo -k 0.4 -s 0.6  # 关键词混淆率40%，句子混淆率60%
node ./bin/obfuscate.js ./demo --no-keyword   # 只启用句子混淆
node ./bin/obfuscate.js ./demo --no-sentence  # 只启用关键词混淆

参数说明：
-o: 输出目录 . 表示直接输出到目标目录的父级
-n: 混淆份数 最大限制10个
-k: 关键词混淆率 (0.0-1.0，默认: 0.4)
-s: 句子混淆率 (0.0-1.0，默认: 0.6)
--no-keyword: 禁用关键词混淆
--no-sentence: 禁用句子混淆
...
<p>外汇交易は重要な投資手法です。バリュー投資やグロース株への投资も人气があります。</p>

<!-- 关键词混淆后效果 -->
<title>片山晃 × <span>日</span><span>本</span>&#8204;株メソッド | 本質×実践の<span data-key="TiDR3mw">投</span>&nbsp;資講座</title>
<p>外汇交易は重要な投&#8204;資手&#65279;法です。バリ&#8205;ュ&#8205;ー<span data-value="IsQMjKT4">投</span>資やグロース株への投资も人气があります。</p>
```
...
<!-- 句子混淆后最终效果 -->
<p><span data-key="abc123">外</span>汇&#8203;交&#8205;易<span class="fx-key">は</span>重要&#8288;な投&#8204;資手&#65279;法で<span>す</span>。バリ&#8205;ュ&#8205;ー<span data-value="IsQMjKT4">投</span>資や<span data-obf="def456">グ</span>ロ&#8203;ース<span>株</span>への投资&#8204;も人气が<span class="ui-elem">あ</span>り&#8205;ます。</p>
```
...
- **HTML结构**: 关键词混淆和句子混淆可能会增加HTML文件大小
- **句子混淆**: 会在HTML中插入大量span标签和不可见字符，可能影响SEO
- **文本选择**: 句子混淆后的文本在复制粘贴时可能包含额外的HTML标签
- **大文件处理**: 超过1MB的文件会被跳过
- **模块系统**: 保护ES6模块的导入/导出
...
### v2.2.0 - 句子混淆功能 📝 **最新**
- 🆕 **句子混淆引擎**: 在关键词混淆基础上增加句子级别的深度混淆
- 📝 **多种混淆方式**: 支持span标签包装、不可见字符插入、混合模式混淆
- 🎯 **智能文本识别**: 精确识别HTML文本节点，跳过代码和脚本内容
- 🧠 **智能跳过机制**: 自动检测并跳过已被关键词混淆的内容，避免重复处理
- ⚙️ **可配置混淆率**: 支持自定义句子混淆概率（-s 参数）
- 🔄 **层次化处理**: 在关键词混淆后执行，形成双重保护机制
- 🛡️ **语义保护**: 不改变文本含义，仅在视觉层面进行混淆
- 📊 **增强统计**: 新增句子混淆统计信息和效果展示
